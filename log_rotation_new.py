...
def create_each_file_dict(file_dict):
...
def count_update_files(task_files_dict, debug_files_dict, error_files_dict):
    if max_backup_file_count >= len(task_files) > 0 and task_files is not {}:
        file_name_count['task.log'] = len(task_files) - 1
    elif len(task_files) == max_backup_file_count + 1:
        delete_last_old_file(task_files_dict)

    if max_backup_file_count >= len(error_files) > 0 and error_files is not {}:
        file_name_count['error.log'] = len(error_files) - 1
    elif len(error_files) == max_backup_file_count + 1:
        delete_last_old_file(error_files_dict)

    if max_backup_file_count >= len(debug_files) > 0 and debug_files is not {}:
        file_name_count['debug.log'] = len(debug_files) - 1
    elif len(debug_files) == max_backup_file_count + 1:
...
def create_new_log(file_name, count):
    ...
    shutil.copyfile(source, dir_path + "/" + datetime.now().strftime(file_name + '%d_%m_%Y.log_' + str(count)))
    # delete_old_log
...
def check_file_capacity(file_dict):
    try:
        for key in file_name_count:
            # print(key)
            if file_dict[key] > file_max_size_kb:
...
def delete_last_old_file(file_dict):
    ...
    for i in range(1, len(new_file_list)):
        os.rename(dir_path + "/" + new_file_list[i], dir_path + "/" + new_file_list[0][:-1] + str(i - 1))
...
def main():
    ...
    task_files_dict, debug_files_dict, error_files_dict = create_each_file_dict(file_dict)
